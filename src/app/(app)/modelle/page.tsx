import Link from "next/link";
import { PageHeader } from "@/components/page-header";
import { Badge } from "@/components/ui/badge";
import { Button, buttonClasses } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Table, TBody, TD, TH, THead, TR } from "@/components/ui/table";
import { artikelName } from "@/lib/artikel-shared";
import { listArtikel } from "@/lib/domain/artikel";
import { formatMoney } from "@/lib/utils";

export default async function ModellePage({
  searchParams,
}: {
  searchParams: Promise<{ q?: string; inaktiv?: string; page?: string }>;
}) {
  const sp = await searchParams;
  const q = sp.q?.trim() ?? "";
  const mitInaktiven = sp.inaktiv === "1";
  const page = Number(sp.page) || 1;

  const { rows, total, pageCount } = await listArtikel({ q, mitInaktiven, modelle: "nur", page });

  const withParams = (patch: Record<string, string | undefined>) => {
    const p = new URLSearchParams();
    const base: Record<string, string> = { q, inaktiv: mitInaktiven ? "1" : "" };
    for (const [k, val] of Object.entries({ ...base, ...patch })) if (val) p.set(k, val);
    const s = p.toString();
    return s ? `/modelle?${s}` : "/modelle";
  };

  return (
    <div>
      <PageHeader
        title="Modelle"
        description={`${total} Modelle`}
        actions={<Link href="/modelle/neu" className={buttonClasses()}>Neues Modell</Link>}
      />

      <div className="mb-4 flex flex-wrap items-center gap-2">
        <form method="get" className="flex items-center gap-2">
          {mitInaktiven ? <input type="hidden" name="inaktiv" value="1" /> : null}
          <Input name="q" defaultValue={q} placeholder="Suche Modell" className="h-8 w-64" />
          <Button size="sm" variant="outline" type="submit">Suchen</Button>
        </form>
        <Link href={withParams({ inaktiv: mitInaktiven ? undefined : "1" })} className={buttonClasses("ghost", "sm")}>
          {mitInaktiven ? "Inaktive ausblenden" : "Inaktive einblenden"}
        </Link>
      </div>

      <Card>
        <Table>
          <THead>
            <TR>
              <TH>Modell</TH>
              <TH>Nr</TH>
              <TH className="text-right">VK EUR</TH>
              <TH className="text-right">VK US</TH>
              <TH>Status</TH>
            </TR>
          </THead>
          <TBody>
            {rows.map((r) => (
              <TR key={r.id} className={r.datensatzInaktiv ? "opacity-50" : ""}>
                <TD className="font-medium">
                  <Link href={`/modelle/${r.id}`} className="hover:underline">{artikelName(r)}</Link>
                </TD>
                <TD className="font-mono text-xs text-neutral-500">{r.artikelNr ?? "–"}</TD>
                <TD className="text-right tabular-nums">{formatMoney(r.vkEur)}</TD>
                <TD className="text-right tabular-nums">{formatMoney(r.vkUs, "USD")}</TD>
                <TD>{r.datensatzInaktiv ? <Badge tone="amber">inaktiv</Badge> : <Badge tone="green">aktiv</Badge>}</TD>
              </TR>
            ))}
            {rows.length === 0 ? (
              <TR><TD colSpan={5} className="py-6 text-center text-neutral-400">Keine Treffer.</TD></TR>
            ) : null}
          </TBody>
        </Table>
      </Card>

      {pageCount > 1 ? (
        <div className="mt-3 flex items-center justify-between text-sm text-neutral-500">
          <span>Seite {page} / {pageCount}</span>
          <div className="flex gap-2">
            {page > 1 ? <Link href={withParams({ page: String(page - 1) })} className={buttonClasses("outline", "sm")}>Zurück</Link> : null}
            {page < pageCount ? <Link href={withParams({ page: String(page + 1) })} className={buttonClasses("outline", "sm")}>Weiter</Link> : null}
          </div>
        </div>
      ) : null}
    </div>
  );
}
